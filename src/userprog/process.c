...
start_process (void *file_name_)
{
	...
	/* file_name_ is a pointer to a string including the actual file name and the
	 * arguments, we need to get the actual file name out of file_name_*/
	char *savePtr;
...
#define halfWord 4
// Add filename, savePtr to the function.
...
setup_stack (void **esp, const char *fileName, char **savePtr) 
{
	...
	// This part is the revise of original code, put return under if statement.
	if (kpage != NULL) {
	...
	int i = 0;
	int argv_size = 4;
	// Think it as a dynamic array to store the address of each argument
	...
	for (token = (char *) fileName; token != NULL;
		...
		*esp -= strlen (token) + 1;
		...
		// This part is for the args-many tests.
		if (argc >= argv_size){
			argv_size *= 2;
  ...
  if(numOfZero != 0){
	...
	// In order to fill the Argv, or the argv[argc] will be NULL
	argv[argc] = 0;
